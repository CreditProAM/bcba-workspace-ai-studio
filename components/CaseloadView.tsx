...
import { Plus, MoreHorizontal, Activity, MapPin, Search, Filter, RotateCcw } from 'lucide-react';
import { isApiDomain } from '../lib/cutover';
import * as clientsRepo from '../lib/repos/clientsRepo';

type LifecycleFilter = 'active' | 'inactive' | 'discharged' | 'all';
...
export const CaseloadView: React.FC<CaseloadViewProps> = ({
  clients,
  onAddClient,
  onClientClick,
  onClientsChanged,
  ...
  const filteredClients = clients.filter((client) => {
    ...
    const lifecycle = client.lifecycleStatus ?? 'active';
  ...
  const getLifecycleLabel = (lifecycle?: Client['lifecycleStatus']) => {
    if (!lifecycle || lifecycle === 'active') return null;
    return lifecycle.charAt(0).toUpperCase() + lifecycle.slice(1);
  };

  const getLifecycleColor = (lifecycle?: Client['lifecycleStatus']) => {
    switch (lifecycle) {
      case 'inactive':
  ...
  const handleReactivate = async (e: React.MouseEvent, client: Client) => {
    e.stopPropagation();
    if (!isApiDomain('clients') || !client.rowVersion) return;
    setReactivatingId(client.id);
    try {
      await clientsRepo.lifecycleClient(client.id, client, 'active');
      onClientsChanged?.();
    } catch (err) {
      window.alert(err instanceof Error ? err.message : 'Could not reactivate client.');
    } finally {
  ...
  const activeCount = clients.filter(
    (c) => (c.lifecycleStatus ?? 'active') === 'active' && c.status === 'Active',
  ).length;
    ...
    <div className="flex-1 bg-slate-50 p-8 overflow-y-auto">
          ...
          <div className="relative group">
            <button className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors">
              ...
              <span>
                {lifecycleFilter === 'all'
                  ? 'All Lifecycle'
                  : lifecycleFilter.charAt(0).toUpperCase() + lifecycleFilter.slice(1)}
              </span>
            </button>
            <div className="absolute top-full right-0 mt-2 w-44 bg-white border border-slate-200 rounded-xl shadow-xl p-1 hidden group-hover:block z-20">
              {(
                [
                  ['active', 'Active'],
                  ['inactive', 'Inactive'],
                ...
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold hover:bg-slate-50 ${lifecycleFilter === value ? 'text-indigo-600 bg-indigo-50' : 'text-slate-600'}`}
                >
                  {label}
                </button>
              ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredClients.map((client) => {
          const lifecycleLabel = getLifecycleLabel(client.lifecycleStatus);
          ...
          return (
            ...
            >
              <div
                className={`absolute top-0 left-0 w-full h-1.5 ${client.color.replace('bg-', 'bg-').replace('100', '400')}`}
              ></div>
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-4">
                  {client.imageUrl ? (
                    ...
                    <div
                      className={`w-14 h-14 rounded-2xl ${client.color} flex items-center justify-center text-lg font-bold ${client.textColor} border ${client.borderColor}`}
                    >
                      {client.avatar}
                    </div>
                  )}
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <span
                        className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-md border ${getStatusColor(client.status)}`}
                      >
                        {client.status}
                      </span>
                      {lifecycleLabel && (
                        <span
                          className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-md border ${getLifecycleColor(client.lifecycleStatus)}`}
                        >
                          {lifecycleLabel}
                        </span>
                      )}
                      <span className="text-xs text-slate-400 font-medium">
                        ID: #{client.id.toUpperCase().substring(0, 6)}
                      </span>
                    </div>
                  </div>
                </div>
        ...
        <button
          onClick={onAddClient}
          className="rounded-2xl border-2 border-dashed border-slate-200 p-6 flex flex-col items-center justify-center gap-4 text-slate-400 hover:text-indigo-500 hover:border-indigo-200 hover:bg-indigo-50/50 transition-all group min-h-[200px]"
        >
